...
import com.thinkgas.heatapp.databinding.FragmentFeasibilityHomeBinding
import com.thinkgas.heatapp.utils.Status
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class FeasibilityHomeFragment : Fragment() {
    private var _binding: FragmentFeasibilityHomeBinding? = null
    private val binding get() = _binding!!
    private val args by navArgs<FeasibilityHomeFragmentArgs>()
    ...
    override fun onCreateView(
        ...
        _binding = FragmentFeasibilityHomeBinding.inflate(inflater, container, false)
        val params = HashMap<String, String>()
        params["session_id"] = args.sessionId
        params["is_tpi"] = if(AppCache.isTpi) "1" else "0"
        viewModel.getFeasibilityInfo(params)
        ...
        binding.apply {
            if(AppCache.isTpi){
                ...
                tvDone.text = "Feasibility Approval Pending"
                tvPending.text = "Supervisor Feasibility Claimed"
                tvHold.text = "Supervisor Feasibility Hold"
                tvClaim.text = "Supervisor Feasibility Unclaimed"
            }
            ivProfile.setOnClickListener {
                ...
                logoutBuilder.setPositiveButton("Yes") { dialogInterface, i ->
                    ...
                    editor.clear()
                    editor.apply()
                    val directions = FeasibilityHomeFragmentDirections.actionFeasibilityHomeFragmentToLoginFragment()
            ...
            cvPending.setOnClickListener {
                if(pendingNo != null && !pendingNo.equals("0")) {
                    val directions =
                        FeasibilityHomeFragmentDirections.actionFeasibilityHomeFragmentToFeasibilityListFragment(
        ...
        viewModel.feasibilityInfoResponse.observe(viewLifecycleOwner) {
            if (it != null) {
                when (it.status) {
                    Status.LOADING -> {
                        setDialog(true)
