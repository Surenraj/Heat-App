    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Agent> {
        ...
        return try {
            ...
            val response = when (status) {
                "pending" -> apiService.getGcClaimedList(tempData)
                "done" -> apiService.getGcDoneList(tempData)
                "hold" -> apiService.getGcHoldList(tempData)
                "unclaimed"->apiService.getGcUnclaimedList(tempData)
                "failed"->apiService.getGcFailedList(tempData)
                "approved"->apiService.getGcApprovedList(tempData)
                "declined"->apiService.getGcDeclinedList(tempData)
                else -> apiService.getGcClaimedList(tempData)
            }
